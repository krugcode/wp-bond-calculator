<script>
  ...
  async function calculateCosts() {
    ...
    try {
      const response = await fetch(`${window.bcAjax.apiUrl}calculate-bond-cost`, {
        method: 'POST',
        headers: {
    ...
    try {
      const response = await fetch(`${window.bcAjax.apiUrl}generate-pdf`, {
        ...
        body: JSON.stringify({
          type: 'bond',
          data: {
            bond_amount: results.bond_amount,
            total: results.total
          },
...
<div class="bond-calculator">
  ...
  {#if results}
    <div class="results-section">
      <div class="cost-breakdown">
        <h4>Bond cost on: {formatCurrency(results.bond_amount)}</h4>
        ...
        <div class="cost-item">
          ...
          <span class="amount">{formatCurrency(results.breakdown.vat.amount)}</span>
        </div>
  ...
  <div class="disclaimer">
    ...
    <ul>
      ...
      <li>Additional lodgement fee payable to the deeds office in respect of each transfer, mortgage bond, cancellation or cession in the sum of R69.00</li>
    </ul>
    ...
    <p><strong>Disclaimer:</strong> All estimated calculations done here are provided for general information purposes only and do not constitute professional advice. We do not warrant the correctness of this information. For more accurate calculations, please contact us.</p>
  </div>
</div>
